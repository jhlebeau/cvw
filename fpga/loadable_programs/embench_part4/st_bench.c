...
st_benchmark (void)
{
  return st_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}
...
static int
verify_st_benchmark (int unused)
{
...
void
run_st_benchmark (void)
{
  uint64_t start;
  uint64_t cycles;
  int result;

  st_initialise_benchmark ();
  st_warm_caches (1);
  ...
  embench_report_result ("st", verify_st_benchmark (result), cycles);
}
