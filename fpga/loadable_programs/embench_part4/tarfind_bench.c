...
static void
tarfind_initialise_benchmark (void)
{
}


static int tarfind_benchmark_body (int rpt);

static void
tarfind_warm_caches (int heat)
{
  tarfind_benchmark_body (heat);
  return;
}


static int
tarfind_benchmark (void)
{
  return tarfind_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}
...
tarfind_benchmark_body (int rpt)
{
  int i, j, p;
  tar_header_t * hdr;
  int found;
...
static int
verify_tarfind_benchmark (int r)
{
  return r == 1;
}
...
void
run_tarfind_benchmark (void)
{
  uint64_t start;
  uint64_t cycles;
  int result;

  tarfind_initialise_benchmark ();
  tarfind_warm_caches (1);

  start = embench_read_mcycle64 ();
  result = tarfind_benchmark ();
  cycles = embench_read_mcycle64 () - start;

  embench_report_result ("tarfind", verify_tarfind_benchmark (result), cycles);
}
