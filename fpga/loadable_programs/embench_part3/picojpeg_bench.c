...
static int
verify_picojpeg_benchmark (int res __attribute ((unused)))
{
...
static void
picojpeg_initialise_benchmark (void)
{
}


static int picojpeg_benchmark_body (int rpt);

static void
picojpeg_warm_caches (int heat)
{
  int res = picojpeg_benchmark_body (heat);
...
static int
picojpeg_benchmark (void)
{
  return picojpeg_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline))
picojpeg_benchmark_body (int rpt)
{
  int i;
...
void
run_picojpeg_benchmark (void)
{
  uint64_t start_cycles;
  uint64_t stop_cycles;
  int res;

  picojpeg_initialise_benchmark ();
  picojpeg_warm_caches (1);

  start_cycles = embench_read_mcycle64 ();
  res = picojpeg_benchmark ();
  stop_cycles = embench_read_mcycle64 ();

  embench_report_result ("Picojpeg", verify_picojpeg_benchmark (res),
