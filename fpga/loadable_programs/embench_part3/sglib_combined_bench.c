...
static int
verify_sglib_combined_benchmark (int res)
{
...
static void
sglib_combined_initialise_benchmark (void)
{
}



static int sglib_combined_benchmark_body (int rpt);

static void
sglib_combined_warm_caches (int heat)
{
  int res = sglib_combined_benchmark_body (heat);
...
static int
sglib_combined_benchmark (void)
{
  return sglib_combined_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline))
sglib_combined_benchmark_body (int rpt)
{
  volatile int cnt;
  int i;
...
void
run_sglib_combined_benchmark (void)
{
  uint64_t start_cycles;
  uint64_t stop_cycles;
  int res;

  sglib_combined_initialise_benchmark ();
  sglib_combined_warm_caches (1);

  start_cycles = embench_read_mcycle64 ();
  res = sglib_combined_benchmark ();
  stop_cycles = embench_read_mcycle64 ();

  embench_report_result ("Sglib-combined",
                         verify_sglib_combined_benchmark (res),
