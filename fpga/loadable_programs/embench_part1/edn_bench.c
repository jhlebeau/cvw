...
static long int mac (const short *a, const short *b, long int sqr, long int *sum);
static void fir (const short array1[], const short coeff[], long int output[]);
static void fir_no_red_ld (const short x[], const short h[], long int y[]);
static long int latsynth (short b[], const short k[], long int n, long int f);
static void iir1 (const short *coefs, const short *input, long int *optr,
                  long int *state);
...
static void
edn_initialise_benchmark (void)
{
}


static int edn_benchmark_body (int rpt);

static void
edn_warm_caches (int heat)
{
  int res = edn_benchmark_body (heat);
...
edn_benchmark (void)
{
  return edn_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline))
edn_benchmark_body (int rpt)
{
  int j;
...
static int
verify_edn_benchmark (int unused)
{
...
void
run_edn_benchmark (void)
{
  uint64_t start_cycles;
  uint64_t stop_cycles;
  int res;

  edn_initialise_benchmark ();
  edn_warm_caches (1);
  ...
  embench_report_result ("Edn", verify_edn_benchmark (res),
