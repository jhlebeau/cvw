...
static int
verify_huffbench_benchmark (int res __attribute ((unused)))
{
...
static void
huffbench_initialise_benchmark (void)
{
}


static int huffbench_benchmark_body (int rpt);

static void
huffbench_warm_caches (int heat)
{
  int res = huffbench_benchmark_body (heat);
...
static int
huffbench_benchmark (void)
{
  return huffbench_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline)) huffbench_benchmark_body (int rpt)
{
  int j;
...
void
run_huffbench_benchmark (void)
{
  uint64_t start_cycles;
  uint64_t stop_cycles;
  int res;

  huffbench_initialise_benchmark ();
  huffbench_warm_caches (1);

  start_cycles = embench_read_mcycle64 ();
  res = huffbench_benchmark ();
  stop_cycles = embench_read_mcycle64 ();

  embench_report_result ("Huffbench", verify_huffbench_benchmark (res),
