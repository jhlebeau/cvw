...
static int aha_mont64_benchmark_body (int rpt);

static void
aha_mont64_warm_caches (int heat)
{
  int res = aha_mont64_benchmark_body (heat);
...
static int
aha_mont64_benchmark (void)
{
  return aha_mont64_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline))
aha_mont64_benchmark_body (int rpt)
{
  int i;
  int errors;
...
static void
aha_mont64_initialise_benchmark (void)
{
...
static int
verify_aha_mont64_benchmark (int r)
{
  return 0 == r;
}

void
run_aha_mont64_benchmark (void)
{
  uint64_t start_cycles;
  uint64_t stop_cycles;
  int res;

  aha_mont64_initialise_benchmark ();
  aha_mont64_warm_caches (1);

  start_cycles = embench_read_mcycle64 ();
  res = aha_mont64_benchmark ();
  stop_cycles = embench_read_mcycle64 ();

  embench_report_result ("Aha-mont64", verify_aha_mont64_benchmark (res),
