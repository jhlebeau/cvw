...
static int
verify_nettle_sha256_benchmark (int res __attribute ((unused)))
{
  bool correct = true;
...
static void
nettle_sha256_initialise_benchmark (void)
{
}


static int nettle_sha256_benchmark_body (int rpt);

static void
nettle_sha256_warm_caches (int heat)
{
  int res = nettle_sha256_benchmark_body (heat);
...
static int
nettle_sha256_benchmark (void)
{
  return nettle_sha256_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline))
nettle_sha256_benchmark_body (int rpt)
{
  int i;
...
void
run_nettle_sha256_benchmark (void)
{
  uint64_t start_cycles;
  uint64_t stop_cycles;
  int res;

  nettle_sha256_initialise_benchmark ();
  nettle_sha256_warm_caches (1);

  start_cycles = embench_read_mcycle64 ();
  res = nettle_sha256_benchmark ();
  stop_cycles = embench_read_mcycle64 ();

  embench_report_result ("Nettle-sha256",
                         verify_nettle_sha256_benchmark (res),
