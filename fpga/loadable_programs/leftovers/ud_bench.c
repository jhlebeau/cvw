...
static void
ud_initialise_benchmark (void)
{
}


static int ud_benchmark_body (int rpt);

static void
ud_warm_caches (int heat)
{
  int res = ud_benchmark_body (heat);
...
ud_benchmark (void)
{
  return ud_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);

}


static int __attribute__ ((noinline))
ud_benchmark_body (int rpt)
{
  int  k;
...
void
run_ud_benchmark (void)
{
  uint64_t start;
  uint64_t cycles;
  int result;

  ud_initialise_benchmark ();
  ud_warm_caches (1);
  ...
  embench_report_result ("ud", verify_ud_benchmark (result), cycles);
}
