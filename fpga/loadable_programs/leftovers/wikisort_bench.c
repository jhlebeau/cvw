...
static int
verify_wikisort_benchmark (int res __attribute ((unused)))
{
  Test exp[] = {
...
static void
wikisort_initialise_benchmark (void)
{
}


static int wikisort_benchmark_body (int rpt);

static void
wikisort_warm_caches (int heat)
{
  int res = wikisort_benchmark_body (heat);
...
static int
wikisort_benchmark (void)
{
  return wikisort_benchmark_body (LOCAL_SCALE_FACTOR * CPU_MHZ);
}


static int __attribute__ ((noinline))
wikisort_benchmark_body (int rpt)
{
...
void
run_wikisort_benchmark (void)
{
  uint64_t start;
  uint64_t cycles;
  int result;

  wikisort_initialise_benchmark ();
  wikisort_warm_caches (1);

  start = embench_read_mcycle64 ();
  result = wikisort_benchmark ();
  cycles = embench_read_mcycle64 () - start;

  embench_report_result ("wikisort", verify_wikisort_benchmark (result),
                         cycles);
}
